...
type WidgetStepRecord v a = {view :: v, cont :: Aff a}

data WidgetStep v a = WidgetStepEff  (Effect a)
...
instance widgetMultiAlternative :: ( Monoid v ) => MultiAlternative (Widget v) where
  orr wss = case NEA.fromArray wss of
    ...
    Nothing -> empty
    where
    combine :: forall v' a. Monoid v' => NonEmptyArray (Free (WidgetStep v') a) -> Free (WidgetStep v') a
    combine wfs =
    ...
    combineInner :: forall v' a. Monoid v' => NonEmptyArray (WidgetStepRecord v' (Free (WidgetStep v') a)) -> Array (Free (WidgetStep v') a) -> Free (WidgetStep v') a
    ...
    combineViewsConts :: forall v' a. Monoid v' => NonEmptyArray (WidgetStepRecord v' (Free (WidgetStep v') a)) -> Free (WidgetStep v') a
    combineViewsConts ws = wrap $ WidgetStepView {
        view: foldMap1 _.view ws,
        cont: merge ws (map _.cont ws)
    }

    combineInner1 :: forall v' a. Monoid v' => NonEmptyArray (WidgetStepRecord v' (Free (WidgetStep v') a)) -> NonEmptyArray (Free (WidgetStep v') a) -> Free (WidgetStep v') a
    ...
    merge :: forall v' a. Monoid v' => NonEmptyArray (WidgetStepRecord v' (Free (WidgetStep v') a)) -> NonEmptyArray (Aff (Free (WidgetStep v') a)) -> Aff (Free (WidgetStep v') a)
    merge ws wscs = do
